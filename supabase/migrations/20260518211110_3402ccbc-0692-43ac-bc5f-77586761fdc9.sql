CREATE OR REPLACE FUNCTION public.get_today_dashboard_summary(p_user_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(total_sales_count bigint, total_sales_amount numeric, total_proposals_count bigint, total_fees numeric, salespeople jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  sees_all := user_role IN ('admin', 'backoffice', 'gestor');
  ...
  WITH sales_today AS (
    ...
    WHERE s.sale_date = today_date
      AND (sees_all OR s.salesperson_id = COALESCE(p_user_id, auth.uid()))
  ...
  proposals_today AS (
    ...
    WHERE p.created_at >= today_date::timestamp
      ...
      AND (sees_all OR p.user_id = COALESCE(p_user_id, auth.uid()))
    GROUP BY p.user_id
  ),
  all_salespeople AS (
    SELECT
    ...
    WHERE pr.role = 'vendedor'
      AND (sees_all OR pr.id = COALESCE(p_user_id, auth.uid()))
  )
  SELECT
